...
ptrLin associaParagem(ptrLin pL, ptrPar pP, int pos) {
...
ptrLin adicionarLinha(ptrLin pL, ptrPar Pp, int parTotal) {
    int i = 0, ans = 0, res = 0, flag = 0, pos = 0;
    char cod[5];
    ...
    if (novo == NULL) {
        ...
        if (ans == 1) {
            ...
            do {
                ...
                fflush(stdin);
                res = scanf(" %s", cod);
                if (verificaCod_Paragens(Pp, cod, parTotal, &pos) == 1 && strlen(cod) == 4) {// <- Condição necessária para escolher
            ...
            do {
                ...
                i++;
            } while (strlen(cod) != 4 || verificaCod_Paragens(Pp, cod, parTotal, NULL) == 1);
        }
        pL = associaParagem(novo, Pp, pos);
...
void visualizarLinhas(ptrLin p) {
    ...
    if (p == NULL) {
       ...
       return;
        ...
        while (aux != NULL) {
            wprintf(L"\n| %d |       %d      | %s", j, aux->nParExistentes, aux->nome);
            printf("\n+---+--------------+--------------------+");
            j++;
            aux = aux->prox;
        }
