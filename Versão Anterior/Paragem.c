    ...
    do {
        ...
        if (total > 1) {
            for (int k = 0; k < total; ++k) {
                if (strcmp(tolowerString(aux), tolowerString(p[k].cod)) == 0) {
                    flag = 1;
                } else {
                    flag = 0;
                }
...
ptrPar eliminaPar(ptrPar p, int *total) {
    ...
    do {
        ...
        fflush(stdin);
        scanf(" %s", cod);
        if (verificaCod_Paragens(p, cod, *total, &pos) == 1 && strlen(cod) == 4) {// <- Condição necessária para escolher
            flag = 1;                                                       //    escolher a mensagem de erro
        }
        i++;
    } while (strlen(cod) != 4 || verificaCod_Paragens(p, cod, *total, &pos) == 1);
